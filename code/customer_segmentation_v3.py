...
data_reduced=pd.DataFrame(x_pca)[np.arange(0,10)]
...
if recalculate_tsne:
    ...
    tsne=TSNE(n_components=3,verbose=0, perplexity=perp,n_iter=5000,learning_rate=lr)
    tsne_scale_results=tsne.fit_transform(data_reduced)
    data_reduced_tsne=pd.DataFrame(tsne_scale_results,columns=['tse1','tse2','tse3'])

    data_reduced_tsne.to_csv('../processed_data/data_tsne.csv')
else:
    data_reduced_tsne=pd.read_csv('../processed_data/data_tsne.csv',index_col=0)
#%%
plt.figure(figsize = (10,10))
plt.scatter(data_reduced_tsne.iloc[:,0],data_reduced_tsne.iloc[:,1],alpha=0.25, facecolor='lightslategray',s=1)
plt.xlabel('tsne1')
plt.ylabel('tsne2')
plt.title('lr: {} - perplexity: {}'.format(lr,perp))
plt.savefig('./tsne_images/lr_{:.1f}_perp_{:.1f}.jpg'.format(lr,perp),dpi=200)


#%%
data_pred=data_reduced_tsne
...
for cluster in clusters:
	...
	subset = data_pred[data_pred['pred'] == cluster]
	# create scatter of these samples
	plt.scatter(subset['tse1'], subset['tse3'],s=0.1)
...
#Using model with ideal number of clusters, which were chosen based on metrics plots
groups=6
model = GaussianMixture(n_components=groups)
model.fit(data_pred)
yhat=model.predict(data_pred)
...
for cluster in clusters:
	...
	subset = data_pred[data_pred['pred'] == cluster]
	# create scatter of these samples
	axs[0].scatter(subset['tse1'], subset['tse2'],label=str(cluster),s=0.1)
leg=axs[0].legend()

for i in np.arange(len(leg.legendHandles)):
    leg.legendHandles[i]._sizes =[20]

axs[0].set_xlabel('Principal tsne component 1')
axs[0].set_ylabel('Principal tsne component 2')
fig.suptitle('Gaussian mixture clustering with {} groups, as seen using first two principal components'.format(groups))

for cluster in clusters:
	# get row indexes for samples with this cluster
	subset = data_pred[data_pred['pred'] == cluster]
	# create scatter of these samples
	axs[1].scatter(subset['tse1'], subset['tse3'],label=str(cluster),s=0.1)
leg=axs[1].legend()

for i in np.arange(len(leg.legendHandles)):
    leg.legendHandles[i]._sizes =[20]

axs[1].set_xlabel('Principal tsne component 1')
axs[1].set_ylabel('Principal tsne component 3')

customer_segmentation_dict_gaussian=dict(zip(list(pctg_table.index),yhat))
