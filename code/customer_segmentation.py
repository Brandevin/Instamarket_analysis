...
recalculate_customer_department_pctgs=0
recalculate_metrics_kmeans=0
recalculate_metrics_gaussian=0
...
if recalculate_customer_department_pctgs:
    ...
    #filtering only customers with more than 30 items bought
    ...
    customers_filtered=products_bought_by_customer[products_bought_by_customer['products_bought']>30]['user_id']
...
customer_segmentation_dict=dict(zip(list(pctg_table.index),model.labels_))
...
plt.title('Number of customers by groups obtained through Kmeans')
...
#getting items bought by department.
...
total_row['customer_group']='All'
#%%
#getting likelihood of an item given it belongs to a given group
pivot_table_kmeans_by_customer_group=pd.pivot_table(customer_group_product_count,index='customer_group',columns='department_name',values='order_id').reset_index().rename_axis(None,axis=1)
# %%
#appending a row represent likelihood with no "a priori" condition
pivot_table_kmeans_by_customer_group=pivot_table_kmeans_by_customer_group.append(total_row,ignore_index=True)
pivot_table_kmeans_by_customer_group=pivot_table_kmeans_by_customer_group.set_index('customer_group')
#showing table
pivot_table_kmeans_by_customer_group
#%%
...
columns=pivot_table_kmeans_by_customer_group.columns
my_colors = ['red', 'green', 'blue', 'orange','magenta','yellow','grey','saddlebrown','black']
fig,axs=plt.subplots(7,3,figsize=(12,15),sharex=True)
axs=axs.reshape(-1)
i=0
for col in columns:
    y_values=pivot_table_kmeans_by_customer_group[col]
    axs[i].bar(np.arange(0,len(y_values)),100*y_values,color=my_colors)
    axs[i].set_xticks(np.arange(0,len(y_values)))

    axs[i].set_xticklabels(pivot_table_kmeans_by_customer_group.index,rotation=90)
    axs[i].set_title(col)
    i+=1
while i<21:
...
plt.xlabel('Group')
plt.ylabel('Number of customers in group')
plt.title('Number of customers by groups obtained through Gaussian mixture')
#%%
#assigning customers to groups
order_products['customer_group_gaussian']=list(map(lambda x: customer_segmentation_dict_gaussian[x] if x in customer_segmentation_dict_gaussian else -1,order_products['user_id']))
# %%
# Getting items bought by department, grouped by customer cluster. Plot how much participation each cluster got in each department
customer_group_product_count_pctg = (order_products.groupby(['department_name','customer_group_gaussian'])['order_id'].count()/order_products.groupby(['department_name'])['order_id'].count()).reset_index()

pivot_table_gaussian=pd.pivot_table(customer_group_product_count_pctg,index='department_name',columns='customer_group_gaussian',values='order_id')
pivot_table_gaussian.plot(kind='bar',stacked=True,figsize=(12,8))
plt.title('Percentage of department items bought by customer group')
plt.ylabel('% bought by group')
# %%
# Getting items bought by customer cluster, grouped by department.
customer_group_product_count = (order_products.groupby(['customer_group_gaussian','department_name'])['order_id'].count()/order_products.groupby(['customer_group_gaussian'])['order_id'].count()).reset_index()
#getting items bought by department.
total_pctg_by_dep=(order_products.groupby(['department_name'])['order_id'].count()/order_products['order_id'].count()).reset_index()
total_row=dict(zip(total_pctg_by_dep['department_name'],total_pctg_by_dep['order_id']))
total_row['customer_group_gaussian']='All'
# %%
#getting likelihood of an item given it belongs to a given group
pivot_table_gaussian=pd.pivot_table(customer_group_product_count,index='customer_group_gaussian',columns='department_name',values='order_id').reset_index().rename_axis(None,axis=1)
# %%
#appending a row represent likelihood with no "a priori" condition

pivot_table_gaussian=pivot_table_gaussian.append(total_row,ignore_index=True)
pivot_table_gaussian=pivot_table_gaussian.set_index('customer_group_gaussian')
#showing table

pivot_table_gaussian
# %%
#Ploting probability a posteriori that an item belongs to a department, given the customer cluster to which the customer belongs.

columns=pivot_table_gaussian.columns
my_colors = ['red', 'green', 'blue', 'orange','magenta','yellow','grey','saddlebrown','black']
fig,axs=plt.subplots(7,3,figsize=(12,15),sharex=True)
axs=axs.reshape(-1)
i=0
for col in columns:
    y_values=pivot_table_gaussian[col]
    axs[i].bar(np.arange(0,len(y_values)),100*y_values,color=my_colors)
    axs[i].set_xticks(np.arange(0,len(y_values)))

    axs[i].set_xticklabels(pivot_table_gaussian.index,rotation=90)
    axs[i].set_title(col)
    i+=1
while i<21:
    axs[i].spines['top'].set_visible(False)
